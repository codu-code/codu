...
export const config: HtmlToSlateConfig = {
  elementTags: {
    ...
    iframe: (el) => ({
      ...
      // Set children to prevent child text nodes from being processed
      children: [{ text: "" }],
    }),
    img: (el) => {
      const imgObj = {
      ...
      const width =
        el && getAttributeValue(el, "width")
          ? parseInt(getAttributeValue(el, "width"))
          : null;
    ...
    figure: (el) => {
      ...
      const figCaptionChild = el.children.find(
      ...
      const width =
        imgChild && getAttributeValue(imgChild, "width")
          ? parseInt(getAttributeValue(imgChild, "width"))
          : null;
