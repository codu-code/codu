...
/* ============================================================
   NotFound — the center-column 404. The hero is a tiny working
   shell: type `help`, `ls` the site's pages, `open jobs` to jump
   around. A real easter egg for the off-the-map builder.
...
const seg = (t: string, c?: string): Seg => ({ t, c: c || C.out });
const line = (...parts: Seg[]): Entry => ({ type: "line", parts });
const art = (): Entry => ({ type: "art" });
const gap = (): Entry => ({ type: "gap" });

/* The "404" hero, as a bitmap. Rendered as a CSS-grid of accent squares rather
   than block glyphs (`█`) — JetBrains Mono has no full-block glyph, so a font
   fallback would render it at a non-monospace width and shear the art apart. */
const ASCII_404 = [
  "█   █   █████   █   █",
  "█   █   █   █   █   █",
  "█████   █   █   █████",
...
const NF_404_GRID = ASCII_404.map((row) => [...row].map((ch) => (ch === "█" ? 1 : 0)));
const NF_404_COLS = NF_404_GRID[0].length;
const NF_404_CELLS = NF_404_GRID.flat();
...
function Block404() {
  return (
    <div
      ...
      style={{
        gridTemplateColumns: `repeat(${NF_404_COLS}, 9px)`,
        gridAutoRows: "9px",
      }}
    >
      {NF_404_CELLS.map((on, i) => (
...
function TerminalLine({ entry }: { entry: Entry }) {
  if (entry.type === "gap") return <div style={{ height: "0.85em" }} />;
  if (entry.type === "art") return <Block404 />;
  return (
...
function Terminal({ route, onNav, onSearch, onJoin }: TerminalProps) {
  ...
  const run = (raw: string): Entry[] | "CLEAR" => {
    const cmd = raw.trim();
    if (!cmd) return [];
    ...
    if (n === "coffee" || n === "make")
      return [
        line(
          seg("☕ brewing… ", C.out),
          seg("ERR: this server only serves 404s.", C.err),
        ),
      ];
    if (n === "404") return [art()];
